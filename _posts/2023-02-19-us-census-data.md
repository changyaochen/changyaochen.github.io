...
Recently I had to learn the US Census Bureau data, to explore the demographic information at the ZIP code level.
In the past, I usually Google search for nicely parsed data, _e.g._, total population at each ZIP code,
and chances are there are someone already made the data available. After all:

* The data is free (from the US Census Bureau).
* The size of the data is (usually) small enough to be downloaded (after all there are about
...
## US Census Bureau does not know ZIP code
...
To map the US Census Bureau data at the ZIP code level, the closest geographic entity is the Census Tract (there are about 70,000).
...
Census Tracts as well. I found some convenient [crosswalk files](https://www.huduser.gov/portal/datasets/usps_crosswalk.html)
that provide such mappings.

But there is an easier way. Since 2000, the US Census Bureau introduced the geographic entity of
...
provides the details of how ZCTAs are created. Probably the important aspect to a data consumer is:
...
The treasure trove of the US Census Bureau data, is the [American Community Survey](https://en.wikipedia.org/wiki/American_Community_Survey) (ACS).
...
* The decennial census, well, is a census. It is an one-time, complete count of the population.
* The ACS is an ongoing survey, designed to provide up-to-date information about communities throughout the United States,
...
the former reflects more recent trends, whereas the latter provides smaller margin of errors.

## Accessing the ACS data
To be honest, it is not very intuitive how to access the US Bureau data, at least for someone who is new to it.
...
Each of the table comes with an alphanumerical table identifier (_e.g._, "B06009"), and a long description (_e.g._,
...
The raw data will be in tabular format, and a piece of key information is the data dictionary, _i.e._, what each column means.
...
is the data dictionary, _i.e._, mapping between the table identifier, the name of each column in the tables, and the semantics.
...
...
```
It indicates for the table with id of "[b01001](https://www2.census.gov/programs-surveys/acs/summary_file/2021/table-based-SF/data/5YRData/acsdt5y2021-b01001.dat)",
the columns with names of "B01001_E002" and "B01001_M002" means the **E**stimate and **M**argin of error
for "total population of male", respectively. Note that the injected letter **E** and **M** in the column names.

### Download the table from FTP
...
## Conclusion
Here we describe how to understand and access US Census Bureau data. Hopefully it can save you
