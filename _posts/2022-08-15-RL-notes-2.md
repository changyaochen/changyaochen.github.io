...
In the preceding discussion, there is a policy embedded in the process (hence a **decision** process). If we remove this component, such that the system will transition between different states following its own (stochastic) dynamic, but still allow the agent to collect rewards at each state, we reduce the MDP to the Markov Reward Process (MRP).

One can still write the Bellman equation for state value functions in the case of MRP, but in a more compact way.
